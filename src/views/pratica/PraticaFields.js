...
const Fields = ({
  ...
  useEffect(() => {
    ...
    getSectionLabel(pratica.cr9b3_sezione)
  ...
  const getSectionLabel = (cat) => {
    switch (cat) {
      case 'AD':
        setSectionLabel('Ascolto e Dialogo')
        return 'Ascolto e Dialogo'
      case 'RR':
        setSectionLabel('Ricerca e Riflessione')
        return 'Ricerca e Riflessione'
      case 'CR':
        setSectionLabel('Comunicazione e Restituzione')
        return 'Comunicazione'
      case 'EP':
        setSectionLabel('Evaluation & Planning')
        ...
        <CContainer className="mb-5 mt-3">
          <p>
            <CFormSelect
              aria-label="sezione"
              label="Sezione:"
              value={formData.cr9b3_sezione ? formData.cr9b3_sezione : ''}
              options={[
                { label: 'Ascolto e Dialogo', value: 'AD' },
                { label: 'Ricerca e Riflessione', value: 'RR' },
                { label: 'Comunicazione e Restituzione', value: 'CR' },
                { label: 'Evaluation & Planning', value: 'EP' },
              ]}
              onChange={(e) => {
                setFormData({ ...formData, cr9b3_sezione: e.target.value })
                setPraticaEdits({ ...praticaEdits, cr9b3_sezione: e.target.value })
                setIsModified(true)
              }}
              disabled={isView}
            />
